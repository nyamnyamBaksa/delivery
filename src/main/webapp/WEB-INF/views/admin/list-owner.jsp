			...
			<div class="article">
				...
				<div class="div-table">
					<div class="div-row table-head">
						...
						<!-- div class="div-cell table-head">리뷰개수</div -->
					...
					<c:forEach items="${memberList }" var="row">
						...
						<div class="div-cell">${row.sname }</div>
						<div class="div-cell">${row.saddr }</div>
						...
						<select id="grade"
							...
							<optgroup label="탈퇴">
								<option value="0"
									<c:if test="${row.m_grade eq 0}">selected="selected"</c:if>>탈퇴</option>

							</optgroup>
							<optgroup label="일반">
								<option value="1"
									<c:if test="${row.m_grade eq 1}">selected="selected"</c:if>>일반</option>
							</optgroup>

							<optgroup label="관리자">
								<option value="2"
									<c:if test="${row.m_grade eq 2}">selected="selected"</c:if>>관리자</option>
							</optgroup>
